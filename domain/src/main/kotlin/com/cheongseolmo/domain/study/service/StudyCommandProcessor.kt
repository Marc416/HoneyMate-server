...
open class StudyCommandProcessor(
    ...
    override fun createAppLink(studyCode: String, email: String, appLink: String): String {
        // TODO("앱링크결정이 안돼서 임시로 처리함.")
        val account = accountRepository.findByEmail(email) ?: throw Exception("계정이 없습니다")
        return "${appLink}://join-waiting-room?accountKey=${account.key}&studyCode=${studyCode}"
    }
}